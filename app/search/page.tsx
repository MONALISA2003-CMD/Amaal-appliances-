...
export default async function SearchPage({searchParams}:{searchParams:Promise<{q?:string}>}){
  ...
  return <main className="section search-page"><div className="container">
    <div className="eyebrow">Catalogue search</div><h1 className="page-title">{q?`Results for “${q}”`:'Search Amaal'}</h1>
    <form className="search-large" action="/search"><Search size={20}/><input name="q" defaultValue={q} placeholder="Search by product, brand, model or category" aria-label="Search Amaal catalogue"/><button className="btn" type="submit">Search</button></form>
    ...
    {q && (results.length ? <ProductGrid products={results}/> : <div className="empty-state"><h2>No products found.</h2><p className="muted">Try a broader search or browse the complete catalogue.</p><Link className="btn" href="/shop">View all products</Link></div>)}
  </div></main>
}
